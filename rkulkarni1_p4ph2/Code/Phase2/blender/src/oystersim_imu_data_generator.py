...
def gyro_gen(fs, ref_w, gyro_err, vib_def=None):
    ...
    Args:
        fs: sample frequency, Hz.
        ref_w: nx3 true ang vel data, rad/s.
...
def run_acc_demo():
    ...
    print(f"Output Real Accel Values:\n{real_acc}")
...
# CUSTOM CODE
######################### 
def simulate_imu_data(true_acc, true_gyro, fs=1000):

    #################
    # Error Models
    #################
    # Define accelerometer error parameters (medium accuracy)
    acc_err = {
        'b': np.array([0.0, 0.0, 0.0]),  # Constant bias
        'b_drift': np.array([5.0e-5, 5.0e-5, 5.0e-5]),  # Bias drift
        'b_corr': np.array([100.0, 100.0, 100.0]),  # Correlation time for bias drift
        'vrw': np.array([0.03, 0.03, 0.03]) / np.sqrt(fs)  # Velocity random walk
    }

    # Define gyroscope error parameters (medium accuracy)
    gyro_err = {
        'b': np.array([0.0, 0.0, 0.0]) * np.pi/180,  # Constant bias
        'b_drift': np.array([3.5, 3.5, 3.5]) * np.pi/180/3600.0,  # Bias drift
        'b_corr': np.array([100.0, 100.0, 100.0]),  # Correlation time for bias drift
        'arw': np.array([0.25, 0.25, 0.25]) * np.pi/180/60  # Angle random walk
    }

    ###################
    # Vibration Model
    ###################
    # Define vibration model if any (example: random vibration)
    vib_env = '[0.01 0.02 0.03]-random'  # Example: Random vibration with small amplitude
    vib_def = vib_from_env(vib_env, fs)

    # Generate the accelerometer and gyroscope data
    simulated_acc = acc_gen(fs, true_acc, acc_err, vib_def)
    simulated_gyro = gyro_gen(fs, true_gyro, gyro_err, vib_def)
...
def load_logged_data():
    ...
    data = np.load('./log/states.npy', allow_pickle=True).item()
    return data
...
if __name__ == "__main__":
    ...
    acceleration_components = logged_data['state_derivative'][:, 3:6]
    combined_data = np.hstack((full_state, acceleration_components))
    
    # Acceleration components
    true_angvel = combined_data[:,10:13]
    # Angular Vel components
    true_accel = combined_data[:,13:16]

    
    simulated_acc, simulated_gyro = simulate_imu_data(true_accel, true_angvel)

    # Plotting
    fig, axes = plt.subplots(2, 1, figsize=(10, 10))

    # Plot true data
    axes[0].plot(time_array, true_accel, label='True Acceleration (x, y, z)')
    axes[0].plot(time_array, true_angvel, label='True Angular Velocity (x, y, z)', linestyle='--')
    axes[0].set_title('True Acceleration and Angular Velocity')
    axes[0].set_xlabel('Time (s)')
    axes[0].set_ylabel('Acceleration (m/s²) / Angular Velocity (rad/s)')
    axes[0].legend()

    # Plot simulated data
    axes[1].plot(time_array, simulated_acc, label='Simulated Acceleration (x, y, z)')
    axes[1].plot(time_array, simulated_gyro, label='Simulated Angular Velocity (x, y, z)', linestyle='--')
    axes[1].set_title('Simulated Acceleration and Angular Velocity')
    axes[1].set_xlabel('Time (s)')
    axes[1].set_ylabel('Acceleration (m/s²) / Angular Velocity (rad/s)')
    axes[1].legend()
