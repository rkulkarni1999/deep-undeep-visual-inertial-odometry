...
Writer = SummaryWriter(par.logs_dir)
min_loss_t = 1e10
min_loss_v = 1e10
M_deepvo.train()
for ep in range(par.epochs):
	...
	loss_mean = 0
	t_loss_list = []
	for _, t_x, t_y,t_imu in train_dl:
		if use_cuda:
			t_y = t_y.cuda(non_blocking=par.pin_mem)
			t_imu = t_imu.cuda(non_blocking=par.pin_mem)
		ls = M_deepvo.step(t_y,t_imu, optimizer).data.cpu().numpy()
	...
	loss_mean_valid = 0
	v_loss_list = []
	for _, v_x, v_y,v_imu in valid_dl:
		if use_cuda:
			v_y = v_y.cuda(non_blocking=par.pin_mem)
			v_imu = v_imu.cuda(non_blocking=par.pin_mem)
		v_ls = M_deepvo.get_loss(v_y,v_imu).data.cpu().numpy()
		v_loss_list.append(float(v_ls))
		loss_mean_valid += float(v_ls)
	print('Valid take {:.1f} sec'.format(time.time()-st_t))
	loss_mean_valid /= len(valid_dl)
	Writer.add_scalar('Valid loss', loss_mean_valid,ep)
	Writer.flush()
