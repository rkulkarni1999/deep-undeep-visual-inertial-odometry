...
class DeepVO(nn.Module):
    def __init__(self, imsize1, imsize2, batchNorm=True):
        ...
        # RNN
        self.rnn = nn.LSTM(
                    input_size=int(np.prod(__tmp.size())) + 6, 
    ...
    def forward(self, x,imu): 
        ...
        # RNN
        imu_data = imu[:,:-1]
        x = torch.cat((x, imu_data), dim=2)
        out, hc = self.rnn(x.float())
    ...
    def get_loss(self, x, y,imu):
        predicted = self.forward(x,imu)
    ...
    def step(self, x, y,imu, optimizer):
        optimizer.zero_grad()
        loss = self.get_loss(x, y,imu)
        loss.backward()
        if self.clip != None:
